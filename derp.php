...
function db_get_additional_params($connection) {
	...
	if ($connection) {
		$queryHandle = $connection->query('PRAGMA encoding');
		$row = $queryHandle->fetch(PDO::FETCH_NUM);
		$result['PragmaEncoding'] = $row[0];
	}
...
function db_connect_database($host, $port, $username, $password, $database) {
	...
	if (!class_exists('PDO')) {
		global $sqliteerror;
		$sqliteerror = 'Connection failed: '.'PDO is not installed.';
	...
	try {
		...
		while (true) {
			if (isset($_POST['attachdb_name_'. $attachedDbIndex]) && isset($_POST['attachdb_alias_'. $attachedDbIndex])) {
				...
				try {
					$connection->query($attachStatement);

				} catch (Exception $e) {
					global $sqliteerror;
					$sqliteerror = 'Attach failed. "'. $attachStatement.'": '. $e->getMessage();
		...
		global $sqliteerror;
		$sqliteerror = 'Connection failed: '. $e->getMessage();
	}
...
function fetchResult($connectionHandle, $sql) {
	return ($connectionHandle->query($sql));
}

function fetchRow($resultHandle) {
	return ($resultHandle->fetch(PDO::FETCH_NUM));
}

function getResultNumRows($resultHandle) {
	return ($resultHandle->rowCount());
}
...
function getResultNumColumns($resultHandle) {
	return ($resultHandle->columnCount());
}
...
function getError($connectionHandle) {
	...
	if (! empty($sqliteerror)) {
		...
		if ($connectionHandle) {
			$errorInfo = $connectionHandle->errorInfo();
			return ($errorInfo[2]);
...
function db_last_insert_id($connectionHandle) {
	return ($connectionHandle->lastInsertId());
}

function getSQLiteVersion($connectionHandle) {
	return ($connectionHandle->getAttribute(PDO::ATTR_SERVER_VERSION));
}
...
function getColumnMetadata($resultHandle, $columnOffset) {
	$columnMeta = $resultHandle->getColumnMeta($columnOffset);
	$result ["Name"] = $columnMeta ["name"];
	$result ["Length"] = 8000;
	$result ["Type"] = "string";
...
function GuiTestConnection($host, $port, $uid, $pwd, $dbname, &$dblist, &$dblistError, &$testConnectionResult) {
	...
	if (!empty($dbname)) {
		...
		if ($connectionHandle) {
			$testConnectionResult = "<p class=\"successful_info\">Connection to <b>". $dbname ."</b> successful</p><p>Server version: ". getServerVersion($connectionHandle)."</p>";
			db_close($connectionHandle);
...
<html>
	...
	<body onload="CheckNotEmpty()">
		<div align="center">
			...
			<div style="width:740px;" align="center" >
				<form>
					...
					<table cellspacing="3" cellpadding="7" border="0" width="100%" style="border:solid 1px #6BA8D1;">
						...
						<?php
						if (getNeedHost()) {
							echo( "<tr><td nowrap align=\"right\" width=\"250\"> Host/Server name (or IP) <span class=\"marker\">*</span>:</td><td><input type=\"text\" name=\"host\" class=\"inputText\" onkeyup=\"CheckNotEmpty()\" ". $hostMessage ." value=\"". $host ."\"></td></tr>");
						}

						if (getNeedIdentification()) {
							echo ("<tr><td nowrap align=\"right\" >User <span class=\"marker\">*</span>:</td><td><input type=\"text\" name=\"user\" class=\"inputText\" onkeyup=\"CheckNotEmpty()\" ". $uidMessage ." value=\"". @$uid ."\"></td></tr><tr><td nowrap align=\"right\" >Password:</td><td><input type=\"password\" name=\"password\" class=\"inputText\" value=\"". @$pwd ."\"></td></tr>");
						}

						if (getNeedPort()) {
							echo ("<tr><td nowrap align=\"right\" >Port (if not ". (getDefaultPort()) .":</td><td><input type=\"text\" name=\"port\" class=\"inputText\" value=\"". @$port ."\"></td></tr>");
						} ?>
						<tr>
							<td nowrap align="right\" >Database <span class="marker">*</span>:</td>
							<td>
								<?php function CreatePlainDbListControl($color = "", $checkFunction = "", $value = "") {
						...
						<?php if (!empty($dblistError)) { ?>
							<tr>
								<td></td>
								<td><font color="red"><?= ($dblistError); ?></font></td>
							</tr>
						<?php } ?>
						<tr>
							<td></td>
							<td>
								<input type="button" name="btntest" class="submit" value="Test Connection" onClick="this.form.action.value='guitestconnection';this.form.submit();">&nbsp;<input type="button" name="showtables" class="submit" value="ShowTables" onClick="this.form.action.value='showtables';this.form.submit();">
							</td>
						</tr>
						<?php if (!empty($testConnectionResult)) { ?>
							<tr>
								<td></td>
								<td><?= ($testConnectionResult); ?></td>
							</tr>
						<?php } ?>
						<?php if (!empty($tableListError)) { ?>
							<tr>
								<td></td>
								<td><?= ($tableListError ); ?></td>
							</tr>
						<?php } ?>
			...
			<div style="width:700px;" align="right">&copy; 2002-2014 <a href="http://www.sqlmaestro.com/">SQL Maestro Group</a></div>
		</div>
	</body>
</html>
